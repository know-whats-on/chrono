...
async function getAllByPrefix(prefix: string): Promise<any[]> {
  ...
  while (true) {
    ...
    if (error) throw new Error(`getAllByPrefix: ${error.message}`);
...
app.post(`${PREFIX}/public-invoice/:id/comment`, async (c) => {
  try {
    const listId = c.req.param("id");
    const { comment, name } = await c.req.json();
    ...
    const newComment = { text: comment.trim(), name: commenterName, date: new Date().toISOString() };
    ...
    await createNotification(
      list.owner_id, 
      "invoice_comment", 
      `${commenterName} commented on Quote/Invoice for "${list.title}"`, 
...
app.post(`${PREFIX}/send-invoice-email`, async (c) => {
  ...
  try {
    ...
    for (const ci of customItems) subtotal += ci.amount;
...
app.get(`${PREFIX}/calendar-share-requests/incoming`, async (c) => {
  const user = await getUser(c);
  if (!user) return c.json({ error: "Unauthorized" }, 401);
  try { const reqs = await getAllByPrefix(`cal-share-req:to:${user.id}:`); reqs.sort((a: any, b: any) => new Date(b.created_at).getTime() - new Date(a.created_at).getTime()); return c.json(reqs); }
...
app.get(`${PREFIX}/calendar-share-requests/outgoing`, async (c) => {
  const user = await getUser(c);
  if (!user) return c.json({ error: "Unauthorized" }, 401);
  try { const reqs = await getAllByPrefix(`cal-share-req:from:${user.id}:`); reqs.sort((a: any, b: any) => new Date(b.created_at).getTime() - new Date(a.created_at).getTime()); return c.json(reqs); }
...
function notifTypeToCategory(type: string): string {
  ...
  if (type === "friend_shared_cal" || type === "calendar_share_accepted" || type === "calendar_share_rejected") return "friend_calendar_share";
...
function notifTypeToUrl(type: string, meta?: any): string {
  ...
  if ((type === "invoice_viewed" || type === "invoice_accepted" || type === "invoice_comment") && listId) {
  ...
  // Calendar / contacts
  if (type === "friend_requested_cal" || type === "friend_shared_cal" || type === "calendar_share_accepted" || type === "calendar_share_rejected") {
