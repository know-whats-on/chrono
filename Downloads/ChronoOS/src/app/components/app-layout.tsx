...
const eventHostNav = [
  ...
  { to: "/calendar", icon: CalendarDays, label: "Calendar" },
  { to: "/open-events", icon: Users, label: "Events" },
...
export function AppLayout() {
  ...
  const [appMode, setAppModeState] = React.useState(() => localStorage.getItem("chrono_mode") || "business");

  React.useEffect(() => {
    const handleStorageChange = () => {
      setAppModeState(localStorage.getItem("chrono_mode") || "business");
    };
  ...
  const navItems = appMode === "host" ? eventHostNav : defaultNav;
        ...
        .then((data: any[]) => {
          ...
          if (lastSeenNotifRef.current) {
            ...
            for (const n of newOnes.slice(0, 3)) {
              ...
              const isCalNotif = n.type === "friend_shared_cal" || n.type === "friend_updated_cal" || n.type === "friend_requested_cal";
              const isInvoiceNotif = ["invoice_viewed", "invoice_accepted", "invoice_comment", "invoice_change_requested", "invoice_invalidated"].includes(n.type);
              
              let actionLabel = "View";
              let actionRoute = "/settings?section=updates";
              if (isListNotif) { actionLabel = "View Lists"; actionRoute = "/"; }
              else if (isCalNotif) { actionLabel = "View Calendar"; actionRoute = "/settings?section=contacts"; }
              else if (isInvoiceNotif && (n.meta?.listId || n.meta?.list_id)) { actionLabel = "View Document"; actionRoute = `/invoice-generator/${n.meta.listId || n.meta.list_id}`; }
              else if (isInvoiceNotif) { actionLabel = "View Invoices"; actionRoute = "/settings?section=invoices"; }
