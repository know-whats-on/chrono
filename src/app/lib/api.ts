...
async function request(path: string, options: RequestInit = {}, skipAuth: boolean = false) {
...
export const postInvoiceComment = (listId: string, name: string, comment: string) =>
  request(`/public-invoice/${listId}/comment`, { method: "POST", body: JSON.stringify({ name, comment }) }, true);
export const acceptInvoice = (listId: string) =>
  request(`/public-invoice/${listId}/accept`, { method: "POST" }, true);
