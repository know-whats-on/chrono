...
import { getEvents, createEvent, updateEvent, deleteEvent, editEventInstance, deleteEventInstance, queryAvailability, getRules, getBookingLinks, createBookingLink, getCalendarConnections } from "../lib/api";
import { useAuth } from "../lib/auth-context";
import { copyToClipboard } from "../lib/clipboard";
import { EventDetailsModal } from "./event-details-modal";
import {
  ChevronLeft, ChevronRight, Plus, X, Trash2, Calendar as CalendarIcon,
  Eye, EyeOff, Loader2, Focus, Repeat, UtensilsCrossed, Link2, Copy, CheckCircle2, ChevronDown
} from "lucide-react";
import {
  ...
  getDay, isBefore, startOfMonth, endOfMonth, addMonths, subMonths, isSameMonth
} from "date-fns";
import {
...
function getEventStyles(ev: any, connections: any[]) {
  const provider = ev?.provider || "manual";
  const fallback = PROVIDER_COLORS[provider] || PROVIDER_COLORS.manual;
  
  const conn = ev?.connection_id ? connections.find(c => c.id === ev.connection_id) : null;
  const customColor = ev?.color || conn?.color;
  ...
  if (customColor) {
    ...
    return {
      className: "",
      style: {
        backgroundColor: hexToRgba(customColor, 0.85),
        borderLeftColor: hexToRgba(customColor, 0.5),
        color: textColor,
      },
      badgeClassName: "",
      badgeStyle: {
  ...
  return {
    className: `${fallback.bg} ${fallback.border} ${fallback.text}`,
    style: {},
    badgeClassName: fallback.badge,
    badgeStyle: {},
  };
}
...
export function CalendarPage() {
  ...
  const loadEvents = useCallback(async () => {
    try {
      ...
      const [ev, conns] = await Promise.all([
        ...
        <div className="flex items-center justify-between gap-2">
          ...
          <div className="hidden sm:flex items-center gap-1 shrink-0">
            ...
            {(["day", "3day", "week", "month", "agenda"] as ViewMode[]).map((v) => (
              ...
              >
                {v === "day" ? "Day" : v === "3day" ? "3 Days" : v === "week" ? "Week" : v === "month" ? "Month" : "Agenda"}
              </button>
            ))}
          </div>
        </div>
          ...
          <div className="flex items-center gap-1">
            <div className="relative">
              <select
                ...
                className="appearance-none bg-primary text-primary-foreground text-xs font-medium pl-3 pr-8 py-1.5 rounded-lg outline-none cursor-pointer"
              >
              ...
              <div className="absolute right-2.5 top-1/2 -translate-y-1/2 pointer-events-none text-primary-foreground opacity-80">
        ...
        <div className="md:flex-1 md:min-h-0 md:flex md:flex-col">
          {view === "agenda" ? (
            <div className="md:flex-1 md:overflow-y-auto">
              <AgendaView events={events} onEdit={openEdit} onSelect={setSelectedEventId} onDelete={handleDelete} showFreeBusy={showFreeBusy} tz={tz} weather={calendarWeather} connections={connections} />
            </div>
          ) : view === "day" ? (
            <DayView
              day={currentDate}
...
function ProviderBadge({ ev, connections }: { ev: any, connections: any[] }) {
  const provider = ev?.provider || "manual";
  ...
  const styleObj = getEventStyles(ev, connections);
  return (
    <span className={`inline-flex items-center justify-center w-4 h-4 rounded text-[8px] font-bold text-white shrink-0 ${styleObj.badgeClassName}`} style={styleObj.badgeStyle}>
      {label}
    </span>
  );
}
...
function WeekView({ weekDays, events, hours, onEdit, onSelect, onDelete, showFreeBusy, busyBlocks, tz, focusBlocks, mealBlocks, weather, connections }: any) {
                  ...
                  {allDayEvs.map((ev: any) => {
                    ...
                    return (
                      <button
                        ...
                        className={`${styleObj.className} rounded px-1 py-0.5 text-[8px] sm:text-[9px] font-medium truncate text-left cursor-pointer hover:opacity-80 transition`}
                        style={styleObj.style}
                        title={showFreeBusy ? "Busy" : ev.title}
                      >
                        <div className="flex items-center gap-0.5 min-w-0">
                          {!showFreeBusy && <ProviderBadge ev={ev} connections={connections} />}
                ...
                {dayEvents.map((ev: any) => {
                  ...
                  return (
                    <button
                      key={ev.id}
                      onClick={() => onSelect(ev.id)}
                      className={`absolute left-0.5 right-0.5 ${styleObj.className} rounded px-1 py-0.5 text-[9px] sm:text-[10px] border-l-2 cursor-pointer hover:opacity-80 transition z-10 overflow-hidden text-left`}
                      style={{ top: `${pos.top}px`, height: `${pos.height}px`, ...styleObj.style }}
                    ...
                    >
                      {showFreeBusy ? (
                        ...
                        <>
                          <div className="flex items-center gap-0.5 min-w-0">
                            <ProviderBadge ev={ev} connections={connections} />
...
function ThreeDayView({ days, events, hours, onEdit, onSelect, onDelete, showFreeBusy, busyBlocks, tz, focusBlocks, mealBlocks, weather, connections }: any) {
  const hourStart = hours[0];
  const scrollRef = useRef<HTMLDivElement>(null);
                  ...
                  {allDayEvs.map((ev: any) => {
                    ...
                    return (
                      <button
                        key={ev.id}
                        onClick={() => onSelect(ev.id)}
                        className={`${styleObj.className} rounded px-1 py-0.5 text-[9px] font-medium truncate text-left cursor-pointer hover:opacity-80 transition`}
                        style={styleObj.style}
                        title={showFreeBusy ? "Busy" : ev.title}
                      >
                        <div className="flex items-center gap-0.5 min-w-0">
                ...
                {dayEvents.map((ev: any) => {
                  ...
                  return (
                    <button
                      key={ev.id}
                      onClick={() => onSelect(ev.id)}
                      className={`absolute left-0.5 right-0.5 ${styleObj.className} rounded px-1 py-0.5 text-[10px] border-l-2 cursor-pointer hover:opacity-80 transition z-10 overflow-hidden text-left`}
                      style={{ top: `${pos.top}px`, height: `${pos.height}px`, ...styleObj.style }}
                      title={showFreeBusy ? "Busy" : `${ev.title}\n${formatTimeInTz(ev.start_at, tz)} – ${formatTimeInTz(ev.end_at, tz)}`}
                    >
                      {showFreeBusy ? (
...
function DayView({ day, events, hours, onEdit, onSelect, onDelete, showFreeBusy, busyBlocks, tz, focusBlocks, mealBlocks, weather, connections }: any) {
  const hourStart = hours[0];
  const timezone = tz || getDeviceTimezone();
            ...
            {allDayEvents.map((ev: any) => {
              ...
              return (
                <button
                  key={ev.id}
                  onClick={() => onSelect(ev.id)}
                  className={`${styleObj.className} rounded px-2 py-0.5 text-[9px] font-medium truncate cursor-pointer hover:opacity-80 transition`}
                  style={styleObj.style}
                  title={showFreeBusy ? "Busy" : ev.title}
                >
                  <div className="flex items-center gap-0.5 min-w-0">
        ...
        <div className="relative">
          ...
          {timedEvents.map((ev: any) => {
            ...
            return (
              <div
                key={ev.id}
                className={`absolute ${styleObj.className} rounded-lg px-2.5 sm:px-3 py-2 text-sm border-l-2 cursor-pointer hover:opacity-80 transition z-10 flex items-start justify-between gap-2`}
                style={{ top: `${pos.top}px`, height: `${Math.max(pos.height, 32)}px`, left: "60px", right: "8px", ...styleObj.style }}
                onClick={() => onSelect(ev.id)}
              >
...
function MonthView({ currentDate, events, onEdit, onSelect, tz, weather, connections }: any) {
  ...
  return (
    <div className="flex flex-col h-full bg-background min-h-[500px]">
      <div className="grid grid-cols-7 border-b border-border/50 shrink-0">
        {weekDaysHeader.map((d) => (
          <div key={d} className="p-2 text-center text-[10px] sm:text-xs font-medium text-muted-foreground uppercase tracking-wider">
            {d}
          </div>
        ))}
      </div>
      ...
      >
        {days.map((day, i) => {
          ...
          const isToday = isSameDay(day, new Date());
          const dayEvents = events.filter((e: any) => isSameDayInTz(e.start_at, day.toISOString(), tz));
          
          return (
            <div
              key={day.toISOString()}
              className={`min-h-[80px] sm:min-h-[100px] border-b border-r border-border/50 p-1 sm:p-2 flex flex-col gap-1 transition ${
            ...
            >
              ...
              <div className="flex-1 flex flex-col gap-0.5 overflow-hidden">
                  ...
                  return (
                    <div
                      ...
                      className={`text-[10px] px-1 py-0.5 rounded truncate cursor-pointer hover:opacity-80 transition-opacity ${
                        ev.is_all_day ? `${styleObj.badgeClassName} text-white` : `${styleObj.className}`
                      }`}
                      style={ev.is_all_day ? styleObj.badgeStyle : styleObj.style}
                    >
                      {!ev.is_all_day && (
                ...
                {dayEvents.length > 4 && (
                  <div className="text-[10px] text-muted-foreground px-1 mt-0.5 font-medium">
...
function AgendaView({ events, onEdit, onSelect, onDelete, showFreeBusy, tz, weather, connections }: any) {
  const timezone = tz || getDeviceTimezone();
                      ...
                      <div className="contents" key={ev.id}>
                        ...
                        >
                          <div className="flex items-center gap-3 min-w-0 flex-1">
                            <div className={`w-1 self-stretch rounded-full ${styleObj.badgeClassName} shrink-0`} style={styleObj.badgeStyle} />
                            <div className="min-w-0 flex-1">
                              {showFreeBusy ? (
