...
import {
  ...
  getCalendarConnections, connectGoogleCalendar, syncGoogleCalendar, deleteCalendarConnection,
...
import {
  ...
  UtensilsCrossed, FileText, Pencil,
} from "lucide-react";
...
export function SettingsPage() {
  ...
  const sections = [
    ...
    { id: "invoices", label: "My Invoices", icon: FileText, desc: "Manage generated invoices & quotes" },
      ...
      {!activeSection ? (
        <div className="space-y-2">
          ...
          <button
            ...
            className="w-full glass rounded-2xl p-4 flex items-center gap-3 hover:bg-white/10 transition text-left mt-4"
          ...
          <button
            ...
            className="w-full bg-card border rounded-xl p-4 flex items-center gap-3 hover:bg-destructive/5 transition text-left mt-2"
          >
...
function OutlookAccountsCard() {
  ...
  const handleAdd = async () => {
    ...
    setSaving(true);
    try {
      await updateMe({ outlook_accounts: updated });
      await refreshProfile();
      setNewEmail("");
...
function GmailAccountsCard() {
  ...
  const handleAdd = async () => {
    ...
    setSaving(true);
    try {
      await updateMe({ gmail_accounts: updated });
      await refreshProfile();
      setNewEmail("");
      toast.success("Account saved");
    } catch {
  ...
  if (invoices.length === 0) {
    return (
      <div className="glass rounded-3xl p-8 text-center">
        ...
        <p className="text-sm text-muted-foreground mt-1">Generate invoices from your project milestones to see them here.</p>
      </div>
    );
  }
        ...
        return (
          <div key={inv.id} className="glass rounded-2xl p-5 hover:bg-white/40 transition">
            ...
            <div className="mt-4 pt-4 border-t border-black/5 flex items-center justify-between">
              ...
              <button 
                onClick={() => navigate(`/invoice-generator/${inv.id}${inv._isShared ? '?shared=true' : ''}`)}
