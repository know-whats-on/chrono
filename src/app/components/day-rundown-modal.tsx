...
function TimelineEvent({ ev, state, tz, night }: { ev: any; state: "past" | "current" | "future"; tz: string; night?: boolean }) {
  ...
  const dotColor = customColor || style.dot;
  const customTextColor = customColor ? getContrastTextColor(customColor) : undefined;
  const customSubTextColor = customColor ? hexToRgba(customTextColor || "#ffffff", 0.7) : undefined;
  ...
  return (
    <div className={`flex items-start gap-2 py-1 transition-opacity ${isPast ? "opacity-45" : ""}`}>
      ...
      <div className="relative w-6 flex flex-col items-center shrink-0 z-10 pt-2">
        {isCurrent ? (
          <div className="w-3 h-3 rounded-full border-2 shadow-sm" style={{ background: dotColor, borderColor: night ? "rgba(255,255,255,0.3)" : "#fff" }} />
        ) : (
          <div className="w-2 h-2 rounded-full mt-0.5" style={{ background: isPast ? (night ? "#6366f1" : "#c4c4d0") : dotColor, opacity: isPast ? 0.6 : 1 }} />
        )}
      </div>
      ...
      <div
        className={`flex-1 min-w-0 rounded-xl p-2.5 border mb-0.5 transition-all overflow-hidden ${
          isCurrent && !customColor
            ? "shadow-sm ring-1"
            : ""
        } ${isCurrent && !customColor ? style.pill : ""}`}
        style={customColor ? {
          background: hexToRgba(customColor, night ? 0.8 : 0.8),
          border: `1px solid ${hexToRgba(customColor, 0.4)}`,
            ...
            <div className="flex items-center gap-1.5 min-w-0">
              {isCurrent && (
                <span
                  ...
                  style={customColor ? { background: customTextColor, color: customColor } : { background: style.dot, color: "white" }}
                >
                  NOW
                </span>
              )}
              <p className="text-sm font-medium truncate" style={{ color: customColor ? customTextColor : isPast ? (night ? "#7c7fbb" : "#9ca3af") : (night ? "#e0e7ff" : "#1f2937") }}>
                {ev.title}
              </p>
            </div>
            <div className="flex items-center gap-2 mt-0.5 flex-wrap">
              <p className="text-[11px]" style={{ color: customColor ? customSubTextColor : night ? "#a5b4fc" : "#6b7280" }}>
                {ev.is_all_day ? "All day" : `${formatTimeInTz(ev.start_at, tz)} – ${formatTimeInTz(ev.end_at, tz)}`}
              </p>
              {!ev.is_all_day && <span className="text-[10px]" style={{ color: customColor ? customSubTextColor : night ? "#818cf8" : "#9ca3af" }}>·</span>}
              {!ev.is_all_day && <span className="text-[10px]" style={{ color: customColor ? customSubTextColor : night ? "#818cf8" : "#9ca3af" }}>{formatDurationMins(dur)}</span>}
            </div>
            {ev.location && (
              <div className="flex items-center gap-1 mt-0.5 min-w-0">
                <MapPin className="w-2.5 h-2.5 shrink-0" style={{ color: customColor ? customSubTextColor : night ? "#818cf8" : "#9ca3af" }} />
                <p className="text-[10px] truncate min-w-0" style={{ color: customColor ? customSubTextColor : night ? "#818cf8" : "#9ca3af" }}>{ev.location}</p>
              </div>
            )}
          </div>
          {isPast && (
            <CheckCircle2 className="w-4 h-4 shrink-0 mt-0.5" style={{ color: customColor ? customTextColor : night ? "#6366f1" : "#d1d5db" }} />
          )}
        </div>
      </div>
    </div>
